...
namespace Opm {
    ...
    */
    ...
    template< typename T >
    bool GridProperties<T>::supportsKeyword(const fst::string& keyword) const {
        return m_supportedKeywords.count( keyword ) > 0;
    }

    template< typename T >
    bool GridProperties<T>::hasKeyword( const fst::string& kw ) const {
    ...
    template< typename T >
    const GridProperty<T>& GridProperties<T>::getKeyword( const fst::string& kw ) const {
        if (!hasKeyword(kw))
    ...
    template< typename T >
    const GridProperty<T>& GridProperties<T>::getInitializedKeyword( const fst::string& kw ) const {
        if (hasKeyword(kw))
    ...
    template< typename T >
    bool GridProperties<T>::addKeyword( const fst::string& kw ) {
    ...
    template< typename T >
    void GridProperties<T>::copyKeyword(const fst::string& srcField ,
                     const fst::string& targetField ,
                     const Box& inputBox) {
    ...
    template< typename T >
    GridProperty<T>& GridProperties<T>::getOrCreateProperty(const fst::string& name) {
        if (!hasKeyword(name))
            addKeyword(name);
    ...
    */
    ...
    void GridProperties<T>::handleADDRecord( const DeckRecord& record, BoxManager& boxManager) {
        const fst::string field = record.getItem("field").get< std::string >(0);
    ...
    void GridProperties<T>::handleCOPYRecord( const DeckRecord& record, BoxManager& boxManager) {
        const fst::string srcField = record.getItem("src").get< std::string >(0);
        const fst::string targetField = record.getItem("target").get< std::string >(0);
    ...
    void GridProperties<T>::handleEQUALREGRecord( const DeckRecord& record, const GridProperty<int>& regionProperty ) {
        const fst::string targetArray = record.getItem("ARRAY").get< std::string >(0);
    ...
    void GridProperties<T>::handleCOPYREGRecord( const DeckRecord& record, const GridProperty<int>& regionProperty ) {
        const fst::string srcArray    = record.getItem("ARRAY").get< std::string >(0);
        const fst::string targetArray = record.getItem("TARGET_ARRAY").get< std::string >(0);
    ...
    template< typename T >
    void GridProperties<T>::postAddKeyword(const fst::string& name,
    ...
    template< typename T >
    GridProperty<T>& GridProperties<T>::getKeyword(const fst::string& kw) {
        if (!hasKeyword(kw))
            addAutoGeneratedKeyword_(kw);
    ...
    */
    template< typename T >
    bool GridProperties<T>::addAutoGeneratedKeyword_(const fst::string& keywordName) const {
    ...
    template< typename T >
    bool GridProperties<T>::isAutoGenerated_(const fst::string& keyword) const {
